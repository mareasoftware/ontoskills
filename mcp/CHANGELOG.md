...
- **`resolve_alias` tool** — Case-insensitive alias resolution as 6th MCP tool
...
- **`dependsOnSkill` in JS** — `extractSkillInfo()` regex updated to match `oc:dependsOnSkill` alongside `oc:dependsOn`
