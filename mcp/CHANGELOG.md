...
- **Tool renamed: `ontoskill` → `skill`** — Internal tool name shortened. Claude Code exposes it as `mcp__onto__skill` (server key `onto` + tool name `skill`). The full name `mcp__onto__skill` replaces the previous `mcp__ontoskills__ontoskill`
- **MCP server key shortened: `ontoskills` → `onto`** — The `.mcp.json` `mcpServers` key is now `"onto"` instead of `"ontoskills"`
