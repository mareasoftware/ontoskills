...
function FileTreeLevel({ node, depth, onTtlClick, githubBase }: {
  ...
  return (
    <>
      {depth > 0 && (
        <button onClick={() => setOpen(!open)} className="flex items-center gap-2 w-full px-2 py-1.5 rounded-md hover:bg-white/[0.04] transition-colors text-left" style={{ paddingLeft: `${depth * 16 + 8}px` }}>
          <Chevron open={open} />
