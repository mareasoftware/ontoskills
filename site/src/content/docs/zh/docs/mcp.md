...
OntoMCP 暴露 **1 个统一工具** `mcp__onto__skill`，集技能发现、上下文检索和知识查询于一体。
...
`mcp__onto__skill` 工具取代了旧的多步骤工作流：
...
之后（1 个工具）：
  mcp__onto__skill(q) → 返回上下文或搜索结果
```

1. 代理收到用户请求
2. 调用 `mcp__onto__skill(q: 用户请求)` — 如果查询是技能 ID，立即返回完整上下文；否则返回 BM25 排序的搜索结果
