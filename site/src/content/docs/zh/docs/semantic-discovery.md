...
| **ontoskill** | MCP 工具 — 统一的技能发现和上下文检索（BM25 + 可选语义） |
...
3. ontoskill(q) → 匹配技能ID时返回完整上下文，否则执行关键词/语义搜索
```
...
2. 用户："我需要创建一个 PDF"
   → 智能体调用：ontoskill(q: "create a pdf", top_k: 3)
...
| ontoskill 延迟（BM25） | < 5ms | 手动基准测试 |
| ontoskill 延迟（语义，可选） | < 50ms | 手动基准测试 |
