...
| **mcp__onto__skill** | MCP 工具 — 统一的技能发现和上下文检索（BM25 + 可选语义） |
...
3. mcp__onto__skill(q) → 匹配技能ID时返回完整上下文，否则执行关键词/语义搜索
```
...
### MCP 工具：mcp__onto__skill（统一发现 + 上下文）
...
2. 用户："我需要创建一个 PDF"
   → 智能体调用：mcp__onto__skill(q: "create a pdf", top_k: 3)
   → 返回：匹配技能的完整上下文包含负载、依赖项和知识节点
```
...
| mcp__onto__skill 延迟（BM25） | < 5ms | 手动基准测试 |
| mcp__onto__skill 延迟（语义，可选） | < 50ms | 手动基准测试 |
